...
define_keymap(re.compile("^jetbrains-(?!.*toolbox).*$", re.IGNORECASE),{
    ...
    K("C-Shift-RIGHT_BRACE"): K("Alt-Right"),             # Go to next editor tab
    K("C-Shift-LEFT_BRACE"): K("Alt-Left"),               # Go to previous editor tab
...
#    K("RC-w"):                  K("Alt-w"),           # Close only current tab, instead of all other tabs
    # K("RC-ing to vertical split (Ctrl+Shift+J)
...
define_keymap(re.compile(termStr, re.IGNORECASE),{
    ...
    # Ctrl Tab - In App Tab Switching
    K("LC-Alt-Left") : K("C-Alt-Left"),
    K("LC-Alt-Right") : K("C-Alt-Right"),
    K("LC-Alt-Up") : K("C-Alt-Up"),
